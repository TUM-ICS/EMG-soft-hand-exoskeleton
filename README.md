...
[uv](https://docs.astral.sh/uv/) is a fast Python package installer and resolver. To use it in this repo:
...
2. **Create a virtual environment and install dependencies:**
   ...
   `uv sync` reads `pyproject.toml` and installs all dependencies into `.venv`. Alternatively, with an existing venv activated:
...
3. **Run scripts** e.g. from repo root:
...
To pin versions for reproducibility, commit the lockfile after the first sync:
